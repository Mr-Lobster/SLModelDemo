...
def loadData(data_directory, batch_size, img_H, img_W):
    if os.path.exists('./pickle/test_set.pickle'):
        print("Read the pickle files of test set...")
    ...
    if os.path.exists('./pickle/total_train_set.pickle'):
        print("Read the pickle files of train set...")
        with open('./pickle/total_train_set.pickle', 'rb') as f:
            train_set = pickle.load(f)
    else:
        print("Pickle file of train set do not exist...")
        print("train dataset path: {}".format(os.path.join(data_directory, 'train')))
        imgs_train, labels_train = get_data(imgs_dir=os.path.join(data_directory, 'train'), images_num=960, h=img_H,
                                            w=img_W)
        ...
        with open('./pickle/total_train_set.pickle', 'wb') as f:
...
def train_and_test(train_loader, test_loader, num_classes, img_H, img_W):
    ...
    torch.save(model.state_dict(), './parameter/NNnet_total.pkl')
...
if __name__ == '__main__':
    ...
    print('Start loading...')
    train_loader, test_loader = loadData(data_directory, batch_size, img_H, img_W)
    print('Start training...')
    train_and_test(train_loader, test_loader, num_classes, img_H, img_W)
